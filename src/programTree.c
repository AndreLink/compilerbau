...
nt_program_element_list *       ntf_program_element_list_1(nt_program_element_list * rest, nt_program_element * next)
{
    switch(next->represents)
    {
        case VARIABLE_DECLARATION:
            add_to_ll(rest->variable_declarations, next->content); break;
        case FUNCTION_DECLARATION:
            add_to_ll(rest->function_declarations, next->content); break;
        case FUNCTION_DEFINITION:
            add_to_ll(rest->function_definitions, next->content); break;
        default: break;
    }
    return rest;
}
nt_program_element_list *       ntf_program_element_list_2(nt_program_element * next)
{
    ...
    {
        case VARIABLE_DECLARATION:
            add_to_ll(ret->variable_declarations, next->content); break;
        case FUNCTION_DECLARATION:
            add_to_ll(ret->function_declarations, next->content); break;
        case FUNCTION_DEFINITION:
            add_to_ll(ret->function_definitions, next->content); break;
        default: break;
    }
    return ret;
}
...
nt_program_element *            ntf_program_element_1(nt_variable_declaration * var)
{
    ...
    while(current)
    {
        ...
        type type = var->type;
        if(type == INT_ && ident->arrayAize > 0) type = INTARRAY;
...
nt_function_parameter *         ntf_function_parameter_1(type type, nt_identifier_declaration * identifier)
{
    ...
    ret->type = type;
    if(ret->type == INT_ && identifier->arrayAize > 0) ret->type = INTARRAY;
